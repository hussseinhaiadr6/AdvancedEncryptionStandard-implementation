...
void unsignedCharToU8Array(unsigned char* input, array<uint8_t, 176>& output) {
    for (int i = 0; i < 176; i++) {
        output[i] = input[i];
    }
}
...
void shiftRows(array<array<uint8_t, 4>, 4>& state){
    unsigned char temp1; //use temp variable to store rows during shifting
uint8_t temp = static_cast<uint8_t>(temp1);
...
void add_round_key(array<array<uint8_t, 4>, 4>& state,   array<uint8_t, 16>& round_key) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
...
//cout<<"after the substitutuion method"<<endl;
    SubBytes(state);
	// 	for (int i = 0; i < 4; i++) {
    //     for (int j = 0; j < 4; j++) {
    //         cout << hex << (int)state[i][j] << " ";
    ...
    // }
    shiftRows(state);
// 	cout<<"after the mix rows method"<<endl;
// for (int i = 0; i < 4; i++) {
//         for (int j = 0; j < 4; j++) {
//             cout << hex << (int)state[i][j] << " ";
//         }
...
for (int j = 0; j < 16; j++) {
	...
	// cout<<" the state befor the function is  is :"<<endl;
	// for (int i = 0; i < 4; i++) {
    //     for (int j = 0; j < 4; j++) {
    //         cout << hex << (int)state1[i][j] << " ";
    //     }
